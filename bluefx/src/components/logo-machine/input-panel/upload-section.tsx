  ...
  return (
    <div className="space-y-4">
      ...
      <div>
        ...
        <Card
          className={`p-4 border-2 border-dashed rounded-lg bg-secondary transition-colors cursor-pointer hover:bg-secondary/80 ${
            dragOver === 'reference' ? 'border-primary bg-primary/10' : 'border-border'
          }`}
          onDragOver={(e) => {
            e.preventDefault();
      ...
      <div>
        <div className="flex items-center gap-2 mb-2">
          ...
          <Label className="text-sm font-medium">Face Swap (Optional)</Label>
        </div>
        
        <Card
          className={`p-4 border-2 border-dashed rounded-lg bg-secondary transition-colors cursor-pointer hover:bg-secondary/80 ${
            dragOver === 'face' ? 'border-primary bg-primary/10' : 'border-border'
          }`}
          onDragOver={(e) => {
            e.preventDefault();
            setDragOver('face');
        ...
        >
          {faceSwap?.source_image ? (
            <div className="space-y-3">
              ...
              <div className="flex items-center gap-2">
                <input
                  ...
                  className="rounded"
                />
                <Label htmlFor="apply-to-all" className="text-xs text-muted-foreground">
            ...
            <div className="flex flex-col items-center gap-2 py-4">
              ...
              <div className="text-center">
                <p className="text-sm font-medium">Drop face image or click to upload</p>
                <p className="text-xs text-muted-foreground">Replace faces in generated thumbnails</p>
              </div>
            </div>
          )}
        </Card>
