...
/**
 * Assembly: cut-for-cut rebuild on the ORIGINAL edit rhythm. Each animated
 * clip is generated longer than its scene (Kling minimum 3s), then trimmed
 * back to the source scene duration here — that preserves the ad's pacing,
 * which is most of what makes the original "work".
 *
 * Strategy: normalize each clip (trim, scale/pad, 25fps, aac 48k stereo,
 * silence track if the clip has no audio), then concat-demux with stream
 * copy. Per-clip normalization keeps one bad clip from failing a giant
 * filter graph and makes the concat itself trivial.
 */
...
export async function assembleClips(opts: {
  ...
  const { workDir, clips, width, height, musicFilePath, outPath } = opts;
  ...
  for (const [i, clip] of clips.entries()) {
    ...
    await normalizeClip(clip.filePath, normPath, clip.durationSeconds, evenWidth, evenHeight);
