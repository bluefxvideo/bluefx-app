...
export interface EbookHistoryFilters {
  searchTerm: string;
  filterStatus: string;
  sortOrder: string;
  dateRange: string;
...
interface EbookHistoryFiltersProps {
  onFiltersChange: (filters: EbookHistoryFilters) => void;
}

/**
 * Ebook History Filters - Filter controls for ebook generation history
 * Follows the standardized history filter pattern
 */
export function EbookHistoryFilters({ onFiltersChange }: EbookHistoryFiltersProps) {
  const [filters, setFilters] = useState<EbookHistoryFilters>({
    searchTerm: '',
    filterStatus: 'all',
    sortOrder: 'newest',
    dateRange: 'all'
  ...
  const updateFilters = (newFilters: Partial<EbookHistoryFilters>) => {
    ...
    <TabContentWrapper>
      ...
      <div className="flex items-center gap-4 mb-5">
        ...
        <div>
          ...
          <p className="text-zinc-400 font-medium">Filter and search your ebook projects</p>
        </div>
      </div>
